function J = costFunctionVHIP(X,U,~,data,modelProperties,stateLim,controlLim,stateWeight,controlWeight,controlRateWeight)
...
Q = diag(stateWeight([1 3 4 6]).*[delta^(-2),(zMax-z0)^(-2),xdotMax^(-2),zdotMax^(-2)]);
R = diag(controlWeight([1 3]).*[delta^(-2),zddotMax^(-2)]);
